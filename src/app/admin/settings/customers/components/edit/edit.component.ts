...
export class EditComponent implements OnInit {
  ...
  formGroup: FormGroup;
  config = {};
  model = {id:0, name:'',email:'',address:'',mobile:'',phone:'',category_id:3 , user_id:1};
  messages: any[];
  errors: any[];
  ...
  ngOnInit() {
    ...
    if(this.catId!=''){
      this.settingsService.SearchCustomer(this.catId).subscribe(customer=>{
        if(customer){
          this.model=customer;
        }
      });      
  ...
  edit(){
   ...
   this.crudService.getProvider(this.provider).crudconfig.route_url='customer/customers/';
   return this.crudService.update(this.provider,this.model,{id:this.model.id}).subscribe(response=>{
     ...
     if(response.isSuccess){
        if(data.done){
          _this.settingsService.CUSTOMERS_LIST.push(data.data);
        }
      // _this.messages.push(response.getMessages);

       _this.router.navigateByUrl('/admin/settings/customers/all');
     }else{
