...
export class EditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if(this.catId!=''){
      this.settingsService.SearchCustomerCategory(this.catId).subscribe(category=>{
        if(category){
          this.model=category;
        }
      });      
  ...
  edit(){
   ...
   this.crudService.getProvider(this.provider).crudconfig.route_url='customer/customer-categories/';
   return this.crudService.update(this.provider,this.model,{id:this.model.id}).subscribe(response=>{
     ...
     if(response.isSuccess){
        if(data.done){
          _this.settingsService.CUSTOMER_CATEGORIES.push(data.data);
        }
      // _this.messages.push(response.getMessages);

       _this.router.navigateByUrl('/admin/settings/customer-categories/all');
     }else{
