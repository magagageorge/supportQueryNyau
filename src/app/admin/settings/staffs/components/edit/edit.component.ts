...
export class EditComponent implements OnInit {
  ...
  formGroup: FormGroup;
  config = {};
  model = {id:0, name:'',email:'',position:'', role_id:3, user_id:3, department_id:1};
  messages: any[];
  errors: any[];
  ...
  ngOnInit() {
    ...
    if(this.catId!=''){
      this.settingsService.SearchStaff(this.catId).subscribe(staff=>{
        if(staff){
          this.model=staff;
        }
      });      
  ...
  edit(){
   ...
   this.crudService.getProvider(this.provider).crudconfig.route_url='staff/staffs/';
   return this.crudService.update(this.provider,this.model,{id:this.model.id}).subscribe(response=>{
     ...
     if(response.isSuccess){
        if(data.done){
          _this.settingsService.STAFFS_LIST.push(data.data);
        }
      // _this.messages.push(response.getMessages);

       _this.router.navigateByUrl('/admin/settings/staffs/all');
     }else{
