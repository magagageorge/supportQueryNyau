...
export class EditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if(this.catId!=''){
      this.settingsService.SearchStaffRole(this.catId).subscribe(role=>{
        if(role){
          this.model=role;
        }
      });      
  ...
  edit(){
   ...
   this.crudService.getProvider(this.provider).crudconfig.route_url='staff/staff-roles/';
   return this.crudService.update(this.provider,this.model,{id:this.model.id}).subscribe(response=>{
     ...
     if(response.isSuccess){
        if(data.done){
          _this.settingsService.STAFF_ROLES.push(data.data);
        }
      // _this.messages.push(response.getMessages);

       _this.router.navigateByUrl('/admin/settings/staff-roles/all');
     }else{
